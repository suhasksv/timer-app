  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        brightness: Brightness.light, // Start with light theme
      ),
      darkTheme: ThemeData(
        primarySwatch: Colors.blue,
...
class _TimerHomePageState extends State<TimerHomePage> {
  // --- Timers ---
  Timer? _mainTimer; // For the countdown
  ...
  // --- Timer State ---
  Duration _initialDuration = const Duration(minutes: 5); // Default duration
  ...
  bool _showColons = true; // For blinking effect
  ...
  void _onTimerFinished() {
    // Optional: Play a sound, show a dialog, etc.
    showDialog(
      context: context,
    ...
    // Optionally reset automatically or keep the finished state
            ...
            onPressed: () {
              ...
              if (minutes != null && minutes > 0) {
                setState(() {
                  ...
                  _resetTimer(); // Reset timer with new duration
                });
  ...
  String _formatDuration(Duration duration, {bool showMs = true}) {
    ...
    if (showMs) {
        // Only show MS for laps or potentially a stopwatch mode
    ...
    final colon = _showColons ? ':' : ' '; // Blinking effect
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // Use AnimatedContainer for smooth background transition
      body: AnimatedContainer(
        duration: const Duration(seconds: 2), // Background transition speed
        color: _backgroundColor,
        padding: const EdgeInsets.all(16.0),
        child: SafeArea( // Ensures content isn't obscured by notches/system bars
          child: Column(
            children: [
